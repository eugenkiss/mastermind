-- | The entropy strategy keeps a pool of possible secret codes around. After
...
-- the next round in the average case. If possible this subpool is reduced to
-- consistent guesses.
-- 
...
module Strategies.Entropy (entropy) where
...
getSubPool allAnswers allCodes consistent = 
  let pairs      = [[ (g, entr a g consistent) 
                      | a <- allAnswers ] 
                      | g <- allCodes   ]
      sumPairs   = map (foldr1 $ \(g, e1) (_, e2) -> (g, e1 + e2)) pairs
      sorted     = reverse $ sortBy (comparing snd) sumPairs
      totalSize  = length consistent
      entr a g c = let partSize = length $ getConsistentCodes a g c
                       i | partSize == 0 = 0 
                         | otherwise     = logBase 2 (totalSize /. partSize)
                       p = partSize /. totalSize
                   in  i * p
